...
impl<'a> NodeWidget<'a> {
    // Combines two independently-reasonable sets of constructor args (the
    // hidden_instance/hidden_permanent/selected hide-items state and the
    // channel_state/selected_channel/elapsed_seconds multichannel state) -
    // over clippy's default threshold, but not worth bundling into a
    // sub-struct for a 3-call-site constructor.
    ...
    fn render_channel_rows(
        ...
        .render(header_split[1], buf, mouse_areas);
...
struct VolumeContext {
    ...
    /// See `VolumeWidget::hidden`/`selected` - threaded through so a
    /// hidden/unselected row's text still dims the same way whether it's
    /// rendered via the classic `Unified`/`Stacked` path or `Radiating`'s
    /// `StereoVolumeWidget`.
    hidden: bool,
    selected: bool,
}
...
mod tests {
    ...
    fn node_height_default_is_unaffected_by_channel_count() {
        ...
        assert_eq!(
            NodeWidget::node_height(
                channel_state(ChannelView::Unified),
                &node,
                false
            ),
            NodeWidget::height()
    ...
    #[test]
    fn channels_view_off_ignores_selected_channel() {
        // Regression guard: node_height/rendering must fall back to the
        // ordinary single-bar layout when the view isn't Channels, even
        // if a stale selected_channel is still set from a previous view.
        let fl = libspa_sys::SPA_AUDIO_CHANNEL_FL;
        let fr = libspa_sys::SPA_AUDIO_CHANNEL_FR;
        let node = test_node(Some(vec![fl, fr]), vec![1.0, 0.0]);
        let config = config::Config::from_toml_str("");
    ...
    fn linked_mode_split_marks_every_row_since_the_whole_node_is_selected() {
        ...
        // split even in Linked view (not Channels). There's no
